...
public class MessageTaskUpsertModelValidator : AbstractValidator<MessageTaskUpsertModel>
{
    ...
    {
        RuleFor(model => model.ChannelType).Required().WithMessage("ChannelTypeRequired").When(m => m.Step == 1 && !m.IsDraft);
        RuleFor(model => model.ChannelId).Required().WithMessage("ChannelIdRequired").When(m => m.Step == 1 && !m.IsDraft);
        RuleFor(model => model.EntityId).Required().WithMessage("MessageTemplateRequired").When(m => m.EntityType == MessageEntityTypes.Template && m.Step == 1 && !m.IsDraft);
        ...
        RuleFor(model => model.Receivers).Required().WithMessage("ReceiversRequired").When(m => m.ReceiverType == ReceiverTypes.Assign && m.Step == 2 && !m.IsDraft);

        RuleFor(model => model.Sign).Required().WithMessage("SignRequired").ChineseLetterNumber().Length(2, 12).When(m => m.ChannelType == ChannelTypes.Sms && m.Step == 3 && !m.IsDraft);
