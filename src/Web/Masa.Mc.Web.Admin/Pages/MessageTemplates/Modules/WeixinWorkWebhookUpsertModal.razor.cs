...
public partial class WeixinWorkWebhookUpsertModal : AdminCompontentBase
{
    [Parameter]
    ...
    {
        _model.ChannelType = ChannelTypes.WeixinWorkWebhook;

        if (string.IsNullOrEmpty(_model.Code))
        {
            _model.Code = $"WXWorkWebhook_{UtilConvert.GetGuidToNumber()}";
        }
