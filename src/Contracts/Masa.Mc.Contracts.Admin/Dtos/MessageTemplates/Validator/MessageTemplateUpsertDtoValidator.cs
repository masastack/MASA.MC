...
public class MessageTemplateUpsertDtoValidator : AbstractValidator<MessageTemplateUpsertDto>
{
    ...
    {
        RuleFor(inputDto => inputDto.DisplayName).Required().WithMessage("MessageTemplateDisplayNameRequired")
            .Length(2, 50).WithMessage("MessageTemplateDisplayNameLength");
        RuleFor(inputDto => inputDto.Code).Required().WithMessage("MessageTemplateCodeRequired")
            .LetterNumberSymbol().WithMessage("MessageTemplateCodeLetterNumberSymbol")
            .Length(2, 50).WithMessage("MessageTemplateCodeLength");
        RuleFor(inputDto => inputDto.ChannelId).Required().WithMessage("ChannelIdRequired");
        ...
        RuleFor(inputDto => inputDto.Sign).Required().WithMessage("SignRequired")
        ...
        RuleFor(inputDto => inputDto.TemplateId).Required().WithMessage("MessageTemplateTemplateIdRequired").When(x => x.ChannelType == ChannelTypes.Sms);
        RuleFor(inputDto => inputDto.Title).Required().WithMessage("TitleRequired")
            .Length(2, 50).WithMessage("TitleLength")
            .When(x => x.ChannelType == ChannelTypes.Email || x.ChannelType == ChannelTypes.WebsiteMessage);
        RuleFor(inputDto => inputDto.Content).Required().WithMessage("ContentRequired");
        RuleFor(inputDto => inputDto.JumpUrl).Required().WithMessage("JumpUrlRequired").When(x => x.IsJump);
