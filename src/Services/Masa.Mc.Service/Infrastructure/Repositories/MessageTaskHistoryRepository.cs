...
public class MessageTaskHistoryRepository : Repository<McDbContext, MessageTaskHistory>, IMessageTaskHistoryRepository
{
    ...
    {
        return await (await WithDetailsAsync()).Where(x => x.IsTest == isTest && x.MessageTaskId == messageTaskId && x.Status == MessageTaskHistoryStatuses.WaitSend).OrderBy(x => x.CreationTime).AsTracking().FirstOrDefaultAsync(cancellationToken);
    }
}
