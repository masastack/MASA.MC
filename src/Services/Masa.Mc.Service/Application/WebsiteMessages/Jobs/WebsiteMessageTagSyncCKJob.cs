...
public class WebsiteMessageTagSyncCKJob : BackgroundJobBase<WebsiteMessageTagSyncCKJobArgs>
{
    private readonly IServiceProvider _serviceProvider;

    public WebsiteMessageTagSyncCKJob(ILogger<BackgroundJobBase<WebsiteMessageTagSyncCKJobArgs>>? logger
        , IServiceProvider serviceProvider) : base(logger)
    {
        _serviceProvider = serviceProvider;
    }

    protected override async Task ExecutingAsync(WebsiteMessageTagSyncCKJobArgs args)
    {
        await using var scope = _serviceProvider.CreateAsyncScope();
        var (queryContext, clickHouseContext) = await GetRequiredServiceAsync(scope.ServiceProvider, args.Environment);

        var data = await queryContext.WebsiteMessageTagQueries.Where(x => x.CreationTime < args.Time).ToListAsync();
    ...
    static async Task InsertBatchToClickHouseAsync(McClickHouseContext context, List<WebsiteMessageTagQueryModel> batch)
    {
        await context.Database.EnsureCreatedAsync();

        context.WebsiteMessageTags.AddRange(batch);
